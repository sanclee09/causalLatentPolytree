...
generic if they lie outside a finite union of proper algebraic
hypersurfaces in the parameter space
\[
  \mathbb{R}^{|E|}
  \times\mathbb{R}_{>0}^{|V|}
...
\medskip\noindent
\textbf{Algebraic hypersurface.}
An \emph{algebraic hypersurface} in $\mathbb{R}^{n}$ is the zero set
\[
  \mathcal{V}(p) := \{x \in \mathbb{R}^{n} : p(x) = 0\}
\]
of a single non-trivial polynomial $p$.  Because $p$ is a real-analytic
function that is nonzero on some open set, $\mathcal{V}(p)$ has Lebesgue
measure zero in $\mathbb{R}^n$ (by the analytic implicit function theorem).
A \emph{proper} hypersurface is one that is a strict subset of the
ambient space, i.e.\ $\mathcal{V}(p) \subsetneq \mathbb{R}^{n}$.
Geometrically, a hypersurface is a ``thin'' codimension-one surface: it
has one fewer dimension than the ambient space and cannot contain any
open set.

\medskip\noindent
\textbf{Why the generic set has full measure.}
A finite union of measure-zero sets has measure zero, so the
\emph{non-generic} parameters---forming a finite union of proper algebraic
hypersurfaces---occupy a set of Lebesgue measure zero.  The complementary
\emph{generic} set therefore has full measure.  In particular, if the
parameters $(\Lambda,\sigma^{2},\kappa)$ are drawn from any absolutely
continuous distribution (e.g.\ uniform on a bounded domain), the drawn
configuration is generic with probability one.  Non-generic configurations
correspond to exact algebraic coincidences among structural coefficients
and noise cumulants that would be destroyed by an arbitrarily small
perturbation.  This style of genericity argument is standard in algebraic
statistics \citep{drton:strumfeld:2009}; for the specific algebraic
varieties arising from third-order cumulants of linear non-Gaussian
graphical models, see \citet{amendola:drton:2021}.
\end{remark}
...
(Definition~\ref{def:generic}).  Consequently, Theorem~\ref{thm:identifiability}
...
\item \textbf{Evaluation pipelines}: \texttt{chain\_finite\_sample\_evaluation.py},
