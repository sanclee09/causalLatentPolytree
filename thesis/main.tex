...
% Micro-typography (reduces overfull hbox warnings)
...
\definecolor{TUMGrayDark}    {RGB}{ 88, 88, 90}
\definecolor{TUMGray}        {RGB}{156,157,159}
\definecolor{TUMGrayLight}   {RGB}{217,218,219}
...
% --------------- Title page ---------------
\input{Title_page/classic_title_page}
...
\begin{enumerate}[label=(C\arabic*)]
  ...
  \item Construction of a \emph{cumulant discrepancy matrix}
  (Section~\ref{sec:discrepancy}) and proof that it satisfies the axioms of
  \citet{etesami:kiyavash:coleman:2016} for generic structural parameters
...
\noindent The material in this section largely follows the presentation 
of \citet{tramontano:monod:drton:2022}, adapted to the latent-variable 
...
\begin{sloppypar}
The $k$th cumulant tensor of a random vector $(X_1, \ldots, X_p)$ is the $k$-way tensor in $\mathbb{R}^{p \times \cdots \times p}$ whose entry in position $(i_1, \ldots, i_k)$ is the joint cumulant
\end{sloppypar}
\[
...
\begin{lemma}
\label{lem:tucker}
Let the random vector $X$ follow the LSEM from \eqref{eq:lsem} with noise 
vector $\varepsilon$. Let $\mathcal{C}^{(k)}$ and $\varepsilon^{(k)}$ be the 
$k$th order cumulant tensors of $X$ and $\varepsilon$, respectively. Then
\begin{align*}
    \mathcal{C}^{(k)} &= \varepsilon^{(k)}\bullet 
    \bigl[(I-\Lambda)^{-\top}\bigr]_{j=1}^k
\end{align*}
is the Tucker product of $\varepsilon^{(k)}$ and $k$ copies of 
$(I-\Lambda)^{-\top}$.
\end{lemma}
...
\begin{remark}[Cumulant notation summary]
\label{rem:notation}
For convenience, we collect the cumulant notations used throughout this thesis:
\begin{center}
\begin{tabular}{ll}
\toprule
Notation & Meaning \\
\midrule
$\cum(X_{i_1},\ldots,X_{i_k})$ & Joint cumulant of $X_{i_1},\ldots,X_{i_k}$ (Definition~\ref{def:cumulant_tensor}) \\
$\mathcal{C}^{(k)}_{i_1,\ldots,i_k}$ & Entry $(i_1,\ldots,i_k)$ of the $k$th-order cumulant tensor of $X$ \\
$\mathcal{C}^{(k)}_i$ & Diagonal entry $\mathcal{C}^{(k)}_{i,\ldots,i}$ (Corollary~\ref{cor:simple_trek_2}) \\
$c^{(i,j),k}_m$ & Mixed cumulant $\mathcal{C}^{(k)}_{\underbrace{i,\ldots,i}_{m},\underbrace{j,\ldots,j}_{k-m}}$ \\
\bottomrule
\end{tabular}
\end{center}
\end{remark}
...
Let $G=(V,E)$ be a DAG whose underlying undirected graph is a tree. Partition $V=O\cup L$ into observed and latent vertices. The pair $(G,O)$ is a \emph{minimal latent polytree} if every $\ell\in L$ has out-degree at least~$2$. Equivalently, $(G,O)$ is minimal if it has no \emph{redundant} latent nodes, where a latent node $\ell$ is redundant if the subgraph obtained by removing $\ell$ and its incident edges remains a forest on the remaining nodes \citep{etesami:kiyavash:coleman:2016}.
\end{definition}

The minimality condition ensures that no latent variable is redundant—removing any latent variable would disconnect the observed variables or change the conditional independence structure among them. This constraint is essential for identifiability: any latent node with out-degree 1 can be absorbed into a rescaled edge weight and a modified noise term on its single child, leaving the joint distribution of all observed variables unchanged. The following example illustrates the general mechanism.

Consider, as an illustration, a latent variable $\ell$ with parent $X_j$ and single child $X_i$. The path $X_j \to \ell \to X_i$ induces the same conditional independence relationships among observed variables as the direct edge $X_j \to X_i$, rendering the latent node $\ell$ unidentifiable from observational data alone. Even in the LiNGAM setting with non-Gaussian noise, this equivalence persists at the level of observed variables. Specifically, the model with latent variable
\begin{align*}
X_j &= \varepsilon_j, \\
...
\end{align*}
where the composite noise term $\tilde{\varepsilon}_i := a_2 \varepsilon_\ell + \varepsilon_i$ is non-Gaussian whenever either $\varepsilon_\ell$ or $\varepsilon_i$ is non-Gaussian. Thus, both models induce identical joint distributions over the observed variables $(X_j, X_i)$. Since $\ell$ is entirely absorbed into the noise of $X_i$, no cumulant of any order computed from the observed variables can detect its presence. The same absorption argument applies to any degree-1 latent node regardless of how many parents it has or where it sits in the graph: its contribution always collapses into a rescaled coefficient and a modified noise distribution on its unique child. For the formal general result we refer to \citet{etesami:kiyavash:coleman:2016}; the example above serves only as illustration. This fundamental limitation motivates restricting attention to latent structures where each latent variable has at least two children.
...
\item \textbf{Path uniqueness}: The unique undirected path between any $i,j\in O$ factors through their lowest common ancestor (LCA). This property will underpin our discrepancy construction.
...
\end{equation}
\noindent
where $\Sigma$ is the covariance matrix, $\rho_{i,j} = \text{Corr}[X_i, X_j]$ is the Pearson correlation, and $\mathcal{C}^{(3)}$ is the third-order cumulant tensor.
...
\begin{definition}[Generic parameters]\label{def:generic}
The structural parameters $(\Lambda,\,\sigma^2,\,\kappa)$ of the
latent-LiNGAM polytree model are called \emph{generic} if they do not
satisfy any non-trivial polynomial identity in the edge weights, noise
variances, and noise third cumulants.  Equivalently, the parameters are
generic if they lie outside a proper algebraic subset of the parameter space
\[
  \mathbb{R}^{|E|}
  \times\mathbb{R}_{>0}^{|V|}
  \times(\mathbb{R}\setminus\{0\})^{|V|}.
\]
Since every absolutely continuous distribution on the parameter space
assigns probability one to the generic set, this condition is satisfied
with probability one under any continuous random parameter model.
\end{definition}

\begin{remark}\label{rem:generic}
We unpack the terminology in Definition~\ref{def:generic}.

\medskip\noindent
\textbf{Non-trivial polynomial identity.}
A polynomial $p(\Lambda,\sigma^{2},\kappa)$ is \emph{non-trivial} if it is
not the zero polynomial, i.e.\ it does not vanish for \emph{every} choice
of parameters.  The equation $p=0$ then imposes a genuine algebraic
constraint.  As a concrete instance, the equation arising in the proof of
Axiom~1 (equation~\eqref{eq:axiom1_key}) is non-trivial because it is
linear and non-constant in the noise third cumulant $\kappa_i$: it can
hold for at most one value of $\kappa_i$, not for all values.

\medskip\noindent
\textbf{Proper algebraic subset.}
A \emph{proper algebraic subset} of $\mathbb{R}^{n}$ is the zero set
\[
  \mathcal{V}(p) := \{x \in \mathbb{R}^{n} : p(x) = 0\}
\]
of a single non-trivial polynomial $p$, i.e.\ $\mathcal{V}(p) \subsetneq \mathbb{R}^{n}$.
Note that if finitely many polynomials $p_1, \ldots, p_k$ each define a
hypersurface, their union is captured by the single product polynomial:
$\mathcal{V}(p_1) \cup \cdots \cup \mathcal{V}(p_k) = \mathcal{V}(p_1 \cdots p_k)$.
Thus a finite union of hypersurfaces is itself a proper algebraic subset,
and there is no need to treat unions separately.

\medskip\noindent
\textbf{Why the generic set has full measure.}
Because $p$ is a real-analytic function that is nonzero on some open set,
$\mathcal{V}(p)$ has Lebesgue measure zero in $\mathbb{R}^n$ (by the
analytic implicit function theorem).  The \emph{non-generic} parameters
therefore occupy a set of Lebesgue measure zero, and the complementary
\emph{generic} set has full measure.  In particular, if the parameters
$(\Lambda,\sigma^{2},\kappa)$ are drawn from any absolutely continuous
distribution (e.g.\ uniform on a bounded domain), the drawn configuration
is generic with probability one.  Non-generic configurations correspond to
exact algebraic coincidences among structural coefficients and noise
cumulants that would be destroyed by an arbitrarily small perturbation.
This style of genericity argument is standard in algebraic statistics
\citep{drton:strumfeld:2009}; for the specific algebraic varieties arising
from third-order cumulants of linear non-Gaussian graphical models, see
\citet{amendola:drton:2021}.
\end{remark}

\begin{proposition}[Axioms]\label{prop:axioms}
For generic structural parameters 
(Definition~\ref{def:generic}), the map 
$\gamma:O\times O\to\mathbb R$ defined in 
Definition~\ref{def:discrepancy} satisfies the four axioms of 
Definition~\ref{def:discrepancy_paper1} on the latent--LiNGAM polytree 
model, provided the cumulants exist.
\end{proposition}
...
\begin{enumerate}[label=(\arabic*)]
    \item
    $(\Rightarrow)$
    From Definition~\ref{def:discrepancy}, $\gamma(i,i)=0$. Now assume $i\ne j$ and
    that $i$ is an ancestor of $j$. Then $\mathrm{LCA}(i,j)=i$, and the simple trek
    from $i$ to the pair $(i,j)$ has top $i$, so the simple trek rule gives
    $\Sigma_{i,j}=\lambda^{S}\Sigma_{i,i}$ and
    $\mathcal{C}^{(3)}_{i,i,j}=\lambda^{S}\mathcal{C}^{(3)}_{i}$,
    where $\lambda^{S}$ is the path monomial along the unique directed path $S$ from
    $i$ to $j$.  Hence
    $\Sigma_{i,i}\,\mathcal{C}^{(3)}_{i,i,j}-\Sigma_{i,j}\,\mathcal{C}^{(3)}_{i,i,i}
     =\lambda^{S}\Sigma_{i,i}\mathcal{C}^{(3)}_{i}-\lambda^{S}\Sigma_{i,i}\mathcal{C}^{(3)}_{i}=0$,
    so $\gamma(i,j)=0$.

    $(\Leftarrow)$ Suppose $\gamma(i,j) = 0$ with $i \ne j$.  If $\rho_{i,j} = 0$
then Definition~\ref{def:discrepancy} assigns $\gamma(i,j) = -1 \ne 0$,
a contradiction.  Hence $\rho_{i,j} \ne 0$, and the second case of
Definition~\ref{def:discrepancy} yields
\begin{equation}\label{eq:axiom1_converse}
  \Sigma_{i,i}\,\mathcal{C}^{(3)}_{i,i,j}
  \;=\;
  \Sigma_{i,j}\,\mathcal{C}^{(3)}_{i,i,i}.
\end{equation}

\paragraph{Step 1: Reduce using the Simple Multi-Trek Rule.}
Let $h := \operatorname{LCA}(i,j)$, which exists because $\rho_{i,j}\ne 0$
implies a trek connects $i$ and $j$.  Set $\alpha := \lambda^{P(h,i)}$ and
$\delta := \lambda^{P(h,j)}$ for the path monomials from $h$ to $i$ and $j$
respectively.  Since $h = \operatorname{LCA}(i,j)$, the paths from $h$ to $i$
and from $h$ to $j$ share only the node $h$ (they diverge immediately after
$h$ in the polytree), so the Simple Multi-Trek Rule
(Corollary~\ref{cor:simple-trek-rule}) gives
\[
  \Sigma_{i,j} = \alpha\delta\,\Sigma_{h,h},
  \qquad
  \mathcal{C}^{(3)}_{i,i,j} = \alpha^{2}\delta\,\mathcal{C}^{(3)}_{h},
\]
where $\mathcal{C}^{(3)}_{h} := \mathcal{C}^{(3)}_{h,h,h}$ is the
third-order cumulant of $X_h$.
Since $\rho_{i,j}\ne 0$ implies $\Sigma_{i,j}\ne 0$, and since
$\Sigma_{h,h} = \mathrm{Var}(X_h) > 0$, the product $\alpha\delta\ne 0$,
hence $\alpha\ne 0$ and $\delta\ne 0$.
Substituting into~\eqref{eq:axiom1_converse} and dividing by
$\alpha\delta \ne 0$:
\begin{equation}\label{eq:axiom1_reduced}
  \Sigma_{i,i}\cdot\alpha\cdot\mathcal{C}^{(3)}_{h}
  \;=\;
  \Sigma_{h,h}\cdot\mathcal{C}^{(3)}_{i,i,i}.
\end{equation}

\paragraph{Step 2: Decompose the diagonal quantities.}
By the Multi-Trek Rule (Proposition~\ref{prop:multi:trek}) applied to $X_i$,
the diagonal entries are sums over all ancestors:
\[
  \Sigma_{i,i}
  = \sum_{s\,\in\,\{i\}\cup\mathrm{an}(i)}
      \bigl(\lambda^{P(s,i)}\bigr)^{2}\sigma_{s}^{2},
  \qquad
  \mathcal{C}^{(3)}_{i,i,i}
  = \sum_{s\,\in\,\{i\}\cup\mathrm{an}(i)}
      \bigl(\lambda^{P(s,i)}\bigr)^{3}\kappa_{s},
\]
where $\sigma_s^2 = \mathrm{Var}(\varepsilon_s)$ and
$\kappa_s = \varepsilon^{(3)}_s$ are the noise variance and third cumulant
at node $s$.

Partition $\{i\}\cup\mathrm{an}(i)$ into two disjoint sets:
\begin{itemize}
  \item $A_h := \{h\} \cup \mathrm{an}(h)$, the ancestors of $i$ that
    pass through $h$ (including $h$ itself);
  \item $\mathcal{P}^{*}_{h \to i}$, the nodes strictly on the directed
    path from $h$ to $i$, \emph{excluding} $h$ but \emph{including} $i$.
    This set is empty if and only if $h = i$.
\end{itemize}

For every $s\in A_h$, the directed path to $i$ factors through $h$:
$\lambda^{P(s,i)} = \lambda^{P(s,h)}\cdot\alpha$.  Hence the contributions
from $A_h$ to the two diagonal sums are exactly
\[
  \sum_{s\in A_h}\bigl(\lambda^{P(s,i)}\bigr)^{2}\sigma_s^2
  = \alpha^2\underbrace{\sum_{s\in A_h}\bigl(\lambda^{P(s,h)}\bigr)^{2}
    \sigma_s^2}_{=\,\Sigma_{h,h}},
  \qquad
  \sum_{s\in A_h}\bigl(\lambda^{P(s,i)}\bigr)^{3}\kappa_s
  = \alpha^3\underbrace{\sum_{s\in A_h}\bigl(\lambda^{P(s,h)}\bigr)^{3}
    \kappa_s}_{=\,\mathcal{C}^{(3)}_{h}},
\]
where the underbraced quantities equal $\Sigma_{h,h}$ and
$\mathcal{C}^{(3)}_{h}$ by a second application of
Proposition~\ref{prop:multi:trek} to node $h$.

Define the residual terms from $\mathcal{P}^{*}_{h\to i}$:
\[
  \Delta_{2}
  := \sum_{s\,\in\,\mathcal{P}^{*}_{h\to i}}
     \bigl(\lambda^{P(s,i)}\bigr)^{2}\sigma_{s}^{2},
  \qquad
  \Delta_{3}
  := \sum_{s\,\in\,\mathcal{P}^{*}_{h\to i}}
...
\]
Combining, we obtain
\begin{equation}\label{eq:diag_decomp}
  \Sigma_{i,i} = \alpha^{2}\Sigma_{h,h} + \Delta_{2},
  \qquad
  \mathcal{C}^{(3)}_{i,i,i} = \alpha^{3}\mathcal{C}^{(3)}_{h} + \Delta_{3},
\end{equation}
with $\Delta_2 = \Delta_3 = 0$ if and only if $h = i$.

\paragraph{Step 3: Derive the key equation.}
Substituting~\eqref{eq:diag_decomp} into~\eqref{eq:axiom1_reduced}:
\[
  \bigl(\alpha^{2}\Sigma_{h,h}+\Delta_{2}\bigr)\cdot\alpha
  \cdot\mathcal{C}^{(3)}_{h}
  = \Sigma_{h,h}\cdot
  \bigl(\alpha^{3}\mathcal{C}^{(3)}_{h}+\Delta_{3}\bigr).
\]
Expanding and cancelling the common term
$\alpha^{3}\Sigma_{h,h}\mathcal{C}^{(3)}_{h}$ yields
\begin{equation}\label{eq:axiom1_key}
  \alpha\,\Delta_{2}\,\mathcal{C}^{(3)}_{h}
  \;=\;
  \Sigma_{h,h}\,\Delta_{3}.
\end{equation}

\paragraph{Step 4: Case analysis on $h$.}

\textbf{Case $h = i$.}
Then $\mathcal{P}^{*}_{h\to i} = \varnothing$, so
$\Delta_{2} = \Delta_{3} = 0$, and~\eqref{eq:axiom1_key} holds
trivially.  In this case $i = \operatorname{LCA}(i,j)$, which means $i$
is an ancestor of $j$, as required.

\textbf{Case $h \ne i$.}
The path $h \to \cdots \to i$ has at least one edge, so
$\mathcal{P}^{*}_{h\to i}$ is nonempty and contains node $i$ itself
(as the final node on the path).  The term $s = i$ contributes
$\bigl(\lambda^{P(i,i)}\bigr)^{2}\sigma_i^{2} = \sigma_i^{2} > 0$
to $\Delta_2$, so $\Delta_2 > 0$.

We claim~\eqref{eq:axiom1_key} defines a non-trivial polynomial
equation in the model parameters $(\Lambda,\sigma^{2},\kappa)$, so
that its solution set has measure zero.  To see this, fix all
parameters except $\kappa_i$ (the noise third cumulant at node $i$)
and observe:
\begin{itemize}
  \item \emph{Left side}: $\alpha\,\Delta_{2}\,\mathcal{C}^{(3)}_{h}$
    involves only $\sigma_s^{2}$ (through $\Delta_2$) and $\kappa_t$
    for $t \in A_h$ (through $\mathcal{C}^{(3)}_h$).  Since
    $h \ne i$, node $i$ is a \emph{descendant} of $h$, hence
    $i \notin A_h$.  Thus the left side is \emph{independent of}
    $\kappa_i$.
  \item \emph{Right side}: $\Sigma_{h,h}\,\Delta_{3}$ contains the
    term $\Sigma_{h,h}\cdot\bigl(\lambda^{P(i,i)}\bigr)^{3}\kappa_i
    = \Sigma_{h,h}\,\kappa_i$, so it is \emph{linear and
    non-constant in $\kappa_i$} (since $\Sigma_{h,h} > 0$).
\end{itemize}
Hence the polynomial
$\alpha\,\Delta_{2}\,\mathcal{C}^{(3)}_{h} - \Sigma_{h,h}\,\Delta_{3}$
is not identically zero; it can vanish in $\kappa_i$ for at most one
value.  The set of all parameter configurations satisfying this
equation therefore has Lebesgue measure zero in the full parameter space.

Under the \emph{generic parameters} assumption
(Definition~\ref{def:generic}),
equation~\eqref{eq:axiom1_key} cannot hold when $h\ne i$.
This forces $h = i$, i.e.\ $\operatorname{LCA}(i,j) = i$,
meaning $i$ is an ancestor of $j$.

  \item Let $v_i$ be the lowest common ancestor of two observed variables $v_j$
and $v_k$, and also the lowest common ancestor of $v_j$ and $v_\ell$.
We will show that $\gamma(j,k) = \gamma(j,\ell)$.

By Definition~\ref{def:discrepancy}, for distinct observed vertices $u,v$
with $\rho_{u,v}\neq 0$ one has
\[
  \gamma(u,v)
  = \frac{\mathcal{C}^{(3)}_{u,v,v}\,\Sigma_{u,u}}
         {\mathcal{C}^{(3)}_{u,u,v}\,\Sigma_{u,v}}.
\]
We apply the Simple Multi-Trek Rule (Corollary~\ref{cor:simple-trek-rule})
to the \emph{mixed} cumulant and covariance entries.  Because
$v_i = \operatorname{LCA}(v_j, v_k)$, the unique simple $3$-trek from
$(j,j,k)$ has top $i$: the two paths from $i$ to $j$ are identical (there
is a unique directed path in a polytree), but the path from $i$ to $k$
diverges from the path to $j$ immediately after $i$, so their common
intersection is exactly $\{i\}$, making the trek simple.  Likewise for
the simple 2-trek from $(j,k)$.  This yields
\begin{align*}
  \mathcal{C}^{(3)}_{j,j,k}
    &= \bigl(\lambda^{P(i,j)}\bigr)^2\,\lambda^{P(i,k)}\,\mathcal{C}^{(3)}_{i},
  \\
  \mathcal{C}^{(3)}_{j,k,k}
    &= \lambda^{P(i,j)}\,\bigl(\lambda^{P(i,k)}\bigr)^2\,\mathcal{C}^{(3)}_{i},
  \\
  \Sigma_{j,k}
    &= \lambda^{P(i,j)}\,\lambda^{P(i,k)}\,\Sigma_{i,i},
\end{align*}
where $\lambda^{P(a,b)}$ denotes the product of structural coefficients
along the unique directed path from $a$ to $b$, and
$\mathcal{C}^{(3)}_i$, $\Sigma_{i,i}$ are the third-order cumulant and
variance of $X_{v_i}$, respectively.

\begin{remark*}
We do \emph{not} apply the simple trek rule to the diagonal entry
$\Sigma_{j,j}$.  For a simple 2-trek from top $t\neq j$ to $(j,j)$, both
paths must be the unique directed path from $t$ to $j$; their intersection
then contains every node on that path, not just $t$, so no such simple trek
exists in a polytree.  Consequently $\Sigma_{j,j}$ is treated as a fixed
positive quantity in what follows.
\end{remark*}

Substituting the three identities above into the definition of $\gamma$
gives
\begin{align*}
  \gamma(j,k)
  &= \frac{\mathcal{C}^{(3)}_{j,k,k}\;\Sigma_{j,j}}
          {\mathcal{C}^{(3)}_{j,j,k}\;\Sigma_{j,k}}
  \\[6pt]
  &= \frac{\bigl[\lambda^{P(i,j)}\,\bigl(\lambda^{P(i,k)}\bigr)^2
            \mathcal{C}^{(3)}_i\bigr]\;\Sigma_{j,j}}
          {\bigl[\bigl(\lambda^{P(i,j)}\bigr)^2\,\lambda^{P(i,k)}
            \mathcal{C}^{(3)}_i\bigr]
           \bigl[\lambda^{P(i,j)}\,\lambda^{P(i,k)}\,\Sigma_{i,i}\bigr]}
  \\[6pt]
  &= \frac{\lambda^{P(i,j)}\,\bigl(\lambda^{P(i,k)}\bigr)^2\,
           \mathcal{C}^{(3)}_i\;\Sigma_{j,j}}
          {\bigl(\lambda^{P(i,j)}\bigr)^3\,\bigl(\lambda^{P(i,k)}\bigr)^2\,
           \mathcal{C}^{(3)}_i\,\Sigma_{i,i}}
  \\[6pt]
  &= \frac{\Sigma_{j,j}}
          {\bigl(\lambda^{P(i,j)}\bigr)^2\,\Sigma_{i,i}}.
\end{align*}

An identical computation replacing $v_k$ by $v_\ell$ (with
$\operatorname{LCA}(v_j,v_\ell)=v_i$ by assumption) yields
\[
  \gamma(j,\ell)
  = \frac{\Sigma_{j,j}}
         {\bigl(\lambda^{P(i,j)}\bigr)^2\,\Sigma_{i,i}},
\]
where the right-hand side depends only on the first argument $j$ and on
the shared LCA $i$, not on the second argument.  Hence
\[
  \gamma(j,k) = \gamma(j,\ell),
\]
completing the verification of Axiom~(2).
  ...
  \]
      Because $c$ is an ancestor of $d$, the path from $c$ to $i$ factors through $d$.
  Hence $\lambda^{P(c,i)}=\lambda^{P(c,d)}\,\lambda^{P(d,i)}$ and
  $\bigl(\lambda^{P(c,i)}\bigr)^{2}
   =\bigl(\lambda^{P(c,d)}\bigr)^{2}\bigl(\lambda^{P(d,i)}\bigr)^{2}$.
  Substituting yields
  \[
    \frac{\gamma(i,j)}{\gamma(i,k)}
    =
    \frac{\bigl(\lambda^{P(c,d)}\bigr)^{2}\,\Sigma_{c,c}}{\Sigma_{d,d}}.
  \]
  It remains to show $\bigl(\lambda^{P(c,d)}\bigr)^{2}\Sigma_{c,c} < \Sigma_{d,d}$.
  Since the underlying undirected graph of the polytree is a tree, every non-root
  node has exactly one parent.  Corollary~\ref{cor:simple_trek_2} with $k=2$
  therefore gives the exact single-parent recursion
  \[
    \Sigma_{n,n} = \lambda_{\mathrm{pa}(n),n}^{2}\,\Sigma_{\mathrm{pa}(n),\mathrm{pa}(n)}
                  + \omega_{n}^{2}
    \qquad\text{for every non-root node }n.
  \]
  Let $c=n_{0}\to n_{1}\to\cdots\to n_{m}=d$ be the unique directed path from
  $c$ to $d$.  Applying the recursion $m$ times along this path gives
  \[
    \Sigma_{d,d}
    \;=\;
    \bigl(\lambda^{P(c,d)}\bigr)^{2}\Sigma_{c,c}
    \;+\;
    \sum_{\ell=1}^{m}
      \bigl(\lambda^{P(n_{\ell},d)}\bigr)^{2}\,\omega_{n_{\ell}}^{2},
  \]
  where $\lambda^{P(n_{\ell},d)}$ is the path monomial from $n_{\ell}$ to $d=n_{m}$.
  The term $\ell=m$ contributes $\bigl(\lambda^{P(d,d)}\bigr)^{2}\omega_{d}^{2}
  =\omega_{d}^{2}>0$, so the entire sum is strictly positive.  Consequently,
  $\Sigma_{d,d}>\bigl(\lambda^{P(c,d)}\bigr)^{2}\Sigma_{c,c}$, and therefore
  \[
    \frac{\gamma(i,j)}{\gamma(i,k)}
    =
    \frac{\bigl(\lambda^{P(c,d)}\bigr)^{2}\,\Sigma_{c,c}}{\Sigma_{d,d}}
    < 1,
  \]
  proving that $\gamma(i,j)<\gamma(i,k)$ whenever $\mathrm{LCA}(i,j)$ is
  strictly below $\mathrm{LCA}(i,k)$ on the path from $\mathrm{LCA}(i,k)$ to $i$.
...
\]
\end{definition}
Figure~\ref{fig:tree_merger} demonstrates one such tree merger.
...
Proposition~\ref{prop:axioms} under the \emph{generic parameters} assumption
(Definition~\ref{def:generic}). Since the Separation-Tree-Merger algorithms of
\citet{etesami:kiyavash:coleman:2016} take only the discrepancy matrix as input
and their correctness depends solely on the four axioms being satisfied, the
algorithms apply without modification to our setting; only the discrepancy
measure itself has been replaced.  Consequently, Theorem~\ref{thm:identifiability}
guarantees recovery of the minimal latent polytree whenever the structural
parameters $(\Lambda,\sigma^{2},\kappa)$ are generic---a condition that holds
with probability one under any absolutely continuous parameter distribution.
...
$\hat\Gamma_O$ computed from $n$ observations.  In our setting, the validity of
$\Gamma_O$ as a discrepancy matrix is itself contingent on generic structural
parameters (Proposition~\ref{prop:axioms}); the theorem then applies to all
such parameter configurations.
\end{remark}
...
We evaluate our cumulant-based discrepancy approach through comprehensive experiments on synthetic polytree data. The experimental design proceeds in two stages. We begin with \emph{population-level} experiments, in which the discrepancy matrix $\Gamma_O$ is computed from the true joint distribution rather than estimated from data. This isolates algorithmic performance from estimation noise and directly tests the theoretical guarantees of Theorem~\ref{thm:identifiability}. We then move to \emph{finite-sample} experiments, in which $\Gamma_O$ is replaced by the sample estimator $\hat\Gamma_O$, introducing statistical uncertainty and allowing us to assess practical sample complexity across different polytree topologies.
...
\label{subsubsec:pop_experiments}
...
\item \textbf{Latent identification}: Nodes with out-degree $\geq 2$ are
designated as latent variables.  For the population experiments, we designate
\emph{all} candidate nodes (those with out-degree $\geq 2$) as latent, thereby
creating the maximal latent configuration --- the most demanding scenario
consistent with the minimality condition of Definition~\ref{def:minimal_polytree}.
The finite-sample experiments of Section~\ref{subsec:finite_sample} adopt
the opposite extreme ($k = 1$ latent node) to separately characterise
statistical estimation difficulty; see the discussion there.
\end{itemize}
...
To the best of our knowledge, these are among the largest successful
latent polytree structure learning experiments reported in the literature.
For comparison, \citet{etesami:kiyavash:coleman:2016} demonstrate recovery
on synthetic graphs with up to tens of nodes, and \citet{tramontano:monod:drton:2022}
evaluate on fully observed polytrees of comparable scale; our population-level
experiments extend to $n=1500$ nodes under the latent setting.
...
These findings translate into three concrete guidelines for practitioners:
(1)~edge weights below $|\lambda_{ij}|<0.5$ should be subject to regularisation;
(2)~discrepancy matrix condition numbers exceeding $10^{12}$ signal potential
breakdown; and (3)~for large-scale applications ($n>100$), minimum edge weight
thresholds should satisfy $\eta\geq0.5$.
...
\begin{sloppypar}
The finite-sample discrepancy matrix $\hat\Gamma_O$ is computed via \texttt{compute\_discrepancy\_from\_samples}, which differs from the population routine in three ways driven by estimation noise.
\end{sloppypar}
...
\item \textbf{Structural parameter generation}: Generate random minimal latent polytree with $n$ nodes using Prüfer sequences, assign edge weights $\lambda_{ij} \sim \text{Uniform}[-1,1]$ with $|\lambda_{ij}| \geq \eta$, and sample gamma noise parameters $(k_i, \theta_i)$ as described above.\hfill\mbox{}

\item \textbf{Population reference computation (optional)}: If performing population-referenced evaluation, compute the population observed discrepancy matrix $\Gamma_O$ using the true parameters $((\lambda_{ij}), (k_i, \theta_i))$ via Def.~\ref{def:discrepancy}, restricted to observed nodes.
...
\item \textbf{Latent structure}: Exactly $k = 1$ latent node per polytree,
randomly selected from candidate nodes with out-degree $\geq 2$.  This
constraint simplifies interpretation by isolating the effects of system size
and observed topology on recovery difficulty, while still capturing the
essential challenges of latent variable learning.  Note that this differs
deliberately from the population experiments (Section~\ref{subsubsec:pop_experiments}),
which designated \emph{all} eligible nodes as latent (maximal latent
configuration): in the finite-sample regime, even a single latent node
introduces sufficient estimation difficulty that richer latent configurations
...
These initial results---and in particular the erratic, non-convergent F1 trajectories and the catastrophic corruption concentrated in branching-node rows---pointed strongly to an algorithmic rather than a statistical bottleneck.  A diagnosis of the precise mechanism and the resulting algorithmic refinement, together with re-evaluated results for all three topology classes, are presented in Section~\ref{subsubsec:neartie_reeval}.
...
This initial characterisation---stars as fundamentally unlearnable---and, crucially, the stark disconnect between near-perfect discrepancy matrix accuracy and catastrophic recovery performance, provided the clearest evidence that the failure is algorithmic in nature.  The diagnosis and resulting refinement are detailed in Section~\ref{subsubsec:neartie_reeval}.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{balanced_polytree_analysis_fixed.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{star_polytree_analysis_fixed.pdf}
...
This quadratic growth in the number of simultaneously required constraints explains the
performance degradation and sets a practical ceiling on the fixed-tolerance approach
for large stars.  The degradation is not caused by the tolerance fix itself---it is an
inherent sample-complexity property of pure star structures under any tie-breaking
scheme that requires simultaneous near-uniformity across all pairs.

\subsection{Effect of Latent Node Count on Sample Complexity}
\label{subsec:multi_latent}

\paragraph{Motivation.}
The topology-stratified evaluation above fixes $k=1$ latent node throughout.  The
question of whether the algorithm's convergence properties extend to richer latent
structures is best addressed through the balanced topology.  Chain and star topologies
do not admit valid extensions to $k \geq 2$: any second latent node in a chain would
have out-degree~1, rendering it redundant and violating Definition~\ref{def:minimal_polytree};
a star is definitionally a single latent center with all observed nodes as direct
children \citep{etesami:kiyavash:coleman:2016}, so adding a second latent hub produces
a different topology class entirely.  Balanced structures, by contrast, have multiple
branching points, each of which can naturally host a latent node with out-degree $\geq 2$
while preserving minimality.  The balanced $k \in \{2,3\}$ experiments are therefore
not one option among many but the structurally motivated next step.

All experiments use the same pipeline as the $k=1$ balanced re-evaluation, with the
near-tie tolerance fix ($\rho = 0.05$) applied throughout.  Any remaining failures
therefore reflect finite-sample estimation noise or fundamental structural
non-identifiability, not the algorithmic artefact addressed in
Section~\ref{subsubsec:neartie_reeval}.

\paragraph{Results.}
Tables~\ref{tab:balanced_k2_performance} and~\ref{tab:balanced_k3_performance} report
mean F1-scores over 10 trials for $n \in \{8, 10, 20\}$ and $k \in \{2, 3\}$
respectively.

\begin{table}[h]
\centering
\caption{Balanced polytree recovery performance with $k=2$ latent nodes.
F1-scores averaged over 10 independent trials per configuration.
Edge weights $|\lambda_{ij}| \geq 0.8$; near-tie tolerance $\rho = 0.05$.}
\label{tab:balanced_k2_performance}
\begin{tabular}{lrrr}
\toprule
$n_{\text{samples}}$ & $n=8$ & $n=10$ & $n=20$ \\
\midrule
$10^2$              & 0.32 & 0.16 & 0.08 \\
$10^3$              & 0.30 & 0.17 & 0.04 \\
$10^4$              & 0.41 & 0.28 & 0.10 \\
...
\begin{table}[h]
\centering
\caption{Balanced polytree recovery performance with $k=3$ latent nodes.
F1-scores averaged over 10 independent trials per configuration.
Edge weights $|\lambda_{ij}| \geq 0.8$; near-tie tolerance $\rho = 0.05$.}
\label{tab:balanced_k3_performance}
\begin{tabular}{lrrr}
\toprule
$n_{\text{samples}}$ & $n=8$ & $n=10$ & $n=20$ \\
\midrule
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{balanced_k2_polytree_analysis.pdf}
\caption{Convergence analysis for balanced polytrees with $k=2$ latent nodes
($n \in \{8, 10, 20\}$).
\textbf{Top-left/Top-right/Bottom-left}: Maximum, mean, and median discrepancy
errors (log scale).  All three sizes converge, but $n=20$ starts 2--3 orders of
magnitude higher at 100 samples and does not stabilise until ${\sim}10^5$--$10^6$.
Error floors at large sample sizes reflect zero-population-$\gamma$ pairs that are
structurally irresolvable regardless of sample size.
\textbf{Bottom-right}: F1-scores as a function of sample size.  $n=8$ improves
slowly and plateaus around 0.74 from $10^6$ onward.  $n=10$ climbs more steeply
to ${\sim}0.81$ at $2 \times 10^7$.  $n=20$ is nearly flat until $10^5$, then
exhibits a sharp sample-complexity cliff between $10^5$ and $10^6$, after which
it overtakes the smaller sizes and reaches ${\sim}0.88$.}
\label{fig:balanced_k2_convergence}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{balanced_k3_polytree_analysis.pdf}
\caption{Convergence analysis for balanced polytrees with $k=3$ latent nodes
($n \in \{8, 10, 20\}$).
\textbf{Top-left/Top-right/Bottom-left}: Maximum, mean, and median discrepancy
errors.  $n=20$ shows a dramatic spike in maximum error between $10^3$ and $10^4$
(exceeding $10^3$) before collapsing---numerical instability at low
sample-to-dimension ratios for large sparse graphs.  $n=8$ achieves the lowest
discrepancy errors overall yet the worst F1, demonstrating that low estimation
error does not imply correct structure recovery when the population $\Gamma$ matrix
has many zero entries.
\textbf{Bottom-right}: F1-scores.  $n=8$ peaks at ${\sim}0.47$ around $10^5$ then
degrades to 0.25 at $2 \times 10^7$---the algorithm converges more confidently to
an incorrect structure as data grows.  $n=10$ climbs to ${\sim}0.86$ at $10^7$
then drops slightly to 0.78 at $2 \times 10^7$.  $n=20$ is flat near zero until
$10^4$, shows a sharp cliff between $10^4$ and $10^5$, then improves steadily
to ${\sim}0.80$.}
\label{fig:balanced_k3_convergence}
\end{figure}

\paragraph{Key findings.}

\textbf{$n=8$, $k=3$ is structurally degenerate.}  With only 5 observed nodes and
3 latent nodes, the discrepancy matrix is $5 \times 5$---too sparse to resolve 3
latent sources reliably.  F1 never exceeds 0.47 and degrades at high sample sizes
(Figure~\ref{fig:balanced_k3_convergence}, bottom-right), indicating that the
algorithm converges with increasing confidence to an incorrect structure.  This is a
structural constraint: with $k$ latent nodes each requiring out-degree $\geq 2$, the
minimum number of latent-to-observed edges is $2k$, leaving very little room for
observed-to-observed edges when $n - k$ is small.  For $k=3$, $n \geq 10$ is the
minimum viable graph size.

\textbf{Larger graphs benefit from more constraints at large sample sizes.}  For
$k=2$, F1 at $2 \times 10^7$ is highest for $n=20$ (0.88), then $n=10$ (0.81),
then $n=8$ (0.74).  More observed nodes yield a larger discrepancy matrix with more
non-zero entries, giving the algorithm additional constraints to work with once the
sample size is sufficient.  This counteracts the naive expectation that larger graphs
are always harder.

\textbf{Sharp sample-complexity cliff for $n=20$.}  For both $k=2$ and $k=3$,
$n=20$ shows a dramatic F1 jump between $10^5$ and $10^6$ samples:
\begin{itemize}
  \item $k=2$: $0.31 \; (10^5) \to 0.69 \; (10^6)$
  \item $k=3$: $0.33 \; (10^5) \to 0.70 \; (10^6)$
\end{itemize}
The cliff position is nearly identical for $k=2$ and $k=3$ at $n=20$, suggesting
that sample complexity for large $n$ is driven more by $n$ than by $k$ in this regime.

\textbf{F1 ceiling from fundamental non-identifiability.}  F1 plateaus between 10M
and 20M for many $(n,k)$ combinations.  Since the tolerance fix is applied, this is
not an algorithmic artefact.  It reflects topologies where the algorithm cannot
identify the full structure even with arbitrarily large data.  Concretely, nodes that
share a unique latent common ancestor and are connected only through observed nodes
have zero population $\gamma$ between them.  Both edge orientations within such an
all-observed sub-chain produce the same population discrepancy matrix, making the
direction irresolvable regardless of sample size.  This is consistent with
genericness (Definition~\ref{def:generic}) but reveals a structural ambiguity outside
the reach of the cumulant discrepancy measure.

\textbf{F1 ceiling decreases with $k$.}  The maximum F1 at $2 \times 10^7$ samples
decreases from $k=2$ to $k=3$, most sharply at $n=8$ ($0.74 \to 0.25$) and $n=20$
($0.88 \to 0.80$).  For $n=10$ the ceiling is roughly unchanged ($0.81 \to 0.78$),
suggesting that for medium-sized graphs the algorithm's identifiability is robust to
one additional latent node when the observed count (7 nodes) is sufficient.

\FloatBarrier
\subsection{Summary of Topology-Stratified Findings}

\begin{sloppypar}
The comprehensive finite-sample evaluation across three canonical polytree topologies---chains, balanced branching structures, and stars---reveals that structural properties and algorithmic design choices jointly determine learnability under cumulant-based identification.
\end{sloppypar}
...
is removed.  Chain polytrees with $n=30$ nodes achieve comparable performance
($F1 \approx 0.47$) to unstructured random polytrees with $n=9$ nodes, demonstrating
that exploiting structural constraints enables learning at scales where unconstrained
approaches fail.
...
\begin{itemize}
\item \textbf{Balanced $k=1$, $n \leq 6$}: $n_{\text{samples}} \geq 10^7$ for perfect recovery
\item \textbf{Balanced $k=1$, $n \in \{8, 10\}$}: $n_{\text{samples}} \geq 10^7$ for $F1 > 0.85$
\item \textbf{Balanced $k=2$, $n \in \{8, 10\}$}: $n_{\text{samples}} \geq 10^7$ for $F1 \geq 0.74$; F1 ceiling below 1.0 for some trials due to zero-population-$\gamma$ pairs
\item \textbf{Balanced $k=2$, $n=20$}: sharp cliff at ${\sim}10^6$; $F1 \approx 0.88$ at $2 \times 10^7$
\item \textbf{Balanced $k=3$, $n=8$}: structurally degenerate---F1 peaks at 0.47 then degrades; not reliably learnable
\item \textbf{Balanced $k=3$, $n \in \{10, 20\}$}: $n_{\text{samples}} \geq 10^6$ for $F1 > 0.70$; cliff for $n=20$ at ${\sim}10^5$
...
\item \textbf{Stars, $n = 30$}: $F1 \approx 0.40$ at $2 \times 10^7$; limited by $O(n^2)$ pair-coverage scaling
\item \textbf{Chains, $n \leq 10$}: $n_{\text{samples}} \geq 10^7$ for $F1 > 0.79$
\item \textbf{Chains, $n \geq 20$}: require $> 2 \times 10^7$ samples; endpoint corruption creates a practical ceiling
\end{itemize}
...
\textbf{Structural priors remain important.}  When domain knowledge suggests chain-like
or hierarchical causal structures, cumulant-based polytree learning is viable with
moderate sample sizes.  For balanced and star structures, the requirement that the
algorithm handles near-uniform discrepancy rows correctly is now identified as a key
design criterion.

\textbf{Edge weight thresholds are critical.}  All reported results assume minimum edge
weights $|\lambda_{ij}| \geq 0.8$.  Weaker edges necessitate proportionally larger
samples, with weak thresholds ($\eta \leq 0.3$) causing catastrophic failure even for
simple structures.

\textbf{Topology-stratified validation is essential.}  The dramatic performance
differences across topologies---and the qualitatively different failure modes---
demonstrate that evaluation on random graph ensembles is insufficient for diagnosing
algorithm-specific bottlenecks.

\paragraph{Multi-latent extension.}
Extending the balanced experiments to $k \in \{2,3\}$ latent nodes
(Section~\ref{subsec:multi_latent}) confirms that the algorithm's convergence
behaviour is largely preserved for $n \geq 10$.  The main new findings are: (i)
$n=8$, $k=3$ is structurally degenerate---5 observed nodes are insufficient to
resolve 3 latent sources, and F1 degrades at large sample sizes; (ii) the
sample-complexity cliff for $n=20$ occurs at nearly the same position ($10^5$--$10^6$)
for both $k=2$ and $k=3$, suggesting the cliff is driven by $n$ rather than $k$; and
(iii) F1 plateaus that persist even at $2 \times 10^7$ samples are not algorithmic
artefacts---they reflect topologies where nodes sharing a unique latent ancestor
through an all-observed sub-chain have zero population discrepancy, making edge
orientations irresolvable regardless of sample size.  This constitutes a new class of
non-identifiability beyond the scope of the near-tie fix.
...
\paragraph{Numerical conditioning challenges.} The endpoint corruption in chains
arises from ill-conditioned path products in discrepancy computation. Potential
refinements include:
\begin{itemize}
...
\begin{itemize}
\item Single latent root configurations ($k=1$) for chain and star experiments; $k \in \{1,2,3\}$ for balanced experiments
\item Node counts up to $n=20$ for balanced ($k \in \{2,3\}$ experiments), $n=10$ for balanced ($k=1$), $n=30$ for chains and stars
...
\begin{itemize}
\item \textbf{Multiple latent nodes ($k \geq 2$)}: Balanced topology experiments with
$k \in \{2,3\}$ have been completed (Section~\ref{subsec:multi_latent}), confirming
that the algorithm's convergence behaviour is largely preserved for $n \geq 10$.
Chain and star topologies cannot be extended to $k \geq 2$ while preserving minimality:
any second latent node in a chain would have out-degree~1 (violating Definition~\ref{def:minimal_polytree}),
and a star is definitionally a single latent center with all observed nodes as direct
children \citep{etesami:kiyavash:coleman:2016}---adding a second latent hub yields a
different topology class (flat two-root branching structure) outside the star definition.
Remaining open questions include the behaviour of the algorithm on flat two-root structures
and on chains extended with additional branching.
...
\item \textbf{Partial Gaussianity}: Where some noise terms are Gaussian.
\end{itemize}
...
\item Characterisation of which topology classes admit zero-population-$\gamma$ pairs
  (nodes sharing a latent ancestor through an all-observed sub-chain): a complete
  description would identify exactly which edge orientations are irresolvable by any
  cumulant discrepancy measure, and whether higher-order cumulants could break the ambiguity
\end{itemize}
...
\begin{itemize}
\item \textbf{Topology generation}: \texttt{topology\_stratified\_} \texttt{evaluation.py} implements constrained generation for chains, balanced structures, and stars
...
\item \textbf{Evaluation pipelines}:
  \begin{sloppypar}
  \texttt{chain\_finite\_sample\_evaluation.py},
  \texttt{balanced\_finite\_sample\_evaluation.py},
  \texttt{star\_finite\_sample\_evaluation.py}
  orchestrate topology-specific experiments
  \end{sloppypar}
\item \textbf{Unified experiment runner}: \texttt{run\_experiment.py} provides a single entry point for all topology experiments, reading parameters from \texttt{experiment\_config.yaml}. Each run is invoked as
\begin{verbatim}
python run_experiment.py <topology>
\end{verbatim}
where \texttt{<topology>} is one of \texttt{chain}, \texttt{balanced}, \texttt{star}, \texttt{balanced\_k2}, or \texttt{balanced\_k3}. The runner creates a timestamped output directory under \texttt{experiments/} containing the full console log, a config snapshot, per-trial F1 scores in CSV format, and convergence plots in PDF and PNG.
\end{itemize}

All experiments use fixed random seeds ensuring exact reproducibility. The complete codebase, including experiment configurations and analysis scripts, is available at \url{https://github.com/sanclee09/causalLatentPolytree}.
...
\item Balanced topology experiments with $k \in \{2,3\}$ latent nodes confirm that
convergence behaviour is largely preserved for $n \geq 10$; the F1 ceiling at large
sample sizes reveals a structural non-identifiability (zero-population-$\gamma$ pairs)
that remains open
\end{itemize}

The topology-stratified evaluation framework introduced here---systematically testing
algorithms across chains, balanced structures, and stars---should become standard
practice in structure learning research.  Performance on random graph ensembles obscures
critical topology-dependent behaviour, preventing researchers from understanding when
and why methods succeed or fail.
...
cumulant-based causal discovery.  Other open directions include the rank constraint
methods of \citet{cai:etal:2024:rank} for cases where symmetry-based approaches remain
insufficient, characterisation of which topology classes produce irresolvable zero-population-$\gamma$
pairs, and extension of the multi-latent evaluation to flat two-root branching structures
beyond the balanced class.
