...
from polytree_discrepancy import compute_discrepancy, Polytree
import numpy as np
...
def main(n=8, seed=2025081314):
    ...
    true_obs_edges = {(u,v) for (u,v) in out["edges_directed"] if u in obs and v in obs}
    rec_obs_edges  = {(u,v) for (u,v) in out["recovered_edges"] if (not u.startswith('h')) and (not v.startswith('h'))}
    ...
    mapped_t=set(); mapped_r=set()
    mapping={t:None for t in true_latent_children}
    for score, t, r in pairs:
        if t in mapped_t or r in mapped_r: continue
        if score >= 0.5 or (not true_latent_children[t] and not rec_latent_children[r]):
            mapping[t]=r; mapped_t.add(t); mapped_r.add(r)

    print("\nTrue latent children sets:", {k: sorted(v) for k,v in true_latent_children.items()})
    print("Recovered latent children sets:", {k: sorted(v) for k,v in rec_latent_children.items()})
